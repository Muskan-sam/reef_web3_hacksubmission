...
var _types = require("@acala-network/types");

describe('Scanner', () => {
  let scanner;
  beforeAll(async () => {
    jest.setTimeout(30000000);
    const provider = new _rpcProvider.WsProvider('wss://kusama-rpc.polkadot.io/');
    scanner = new _Scanner.default({
      wsProvider: provider,
      types: _types.types
    });
  ...
  it.only('getChainInfo', async () => {
    const request1 = await scanner.getChainInfo({
      blockNumber: 1
    });
    expect(request1.min).toBe(1);
    expect(request1.max).toBe(1);
    const request2 = await scanner.getChainInfo({
      blockNumber: 2
    });
  ...
  it('getEvents', async () => {
    ...
    const chainInfo = await scanner.getChainInfo({
      blockNumber: 111
    });
